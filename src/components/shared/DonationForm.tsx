...
// import '../../styles/css/DonationForm.css'; // تأكد أن هذا المسار صحيح في مشروعك
...
//     const items = allowed.map(f => ({ id: `${Date.now()}_${Math.random().toString(36).slice(2,8)}`, file: f, url: URL.createObjectURL(f) }));
...
//       images.slice(0, MAX_IMAGES).forEach((it, idx) => fd.append('images', it.file, it.file.name));

//       // If you have donorApi, use it; otherwise the try block simulates network:
//       if (donorApi && typeof donorApi.create === 'function') {
//         await donorApi.create(fd);
//       } else {
//         // simulation (remove when wiring real API)
//         await new Promise(res => setTimeout(res, 900));
//       }

//       // success UX: toast or route
//       setLoading(false);
//       // minimal success microinteraction before routing
//       // clear form and revoke URLs
//       images.forEach(i => URL.revokeObjectURL(i.url));
//       dispatch({ type: 'reset' });
//       setImages([]);
//       setTouched({});
//       // go to dashboard or success page
...
//       setLoading(false);
//     }
//   }, [form, images, setLocation, validateAll]);
...
type FormState = {
  ...
  quantity: number;
  condition: string;
  description: string; // ✅ FIX Bug #3: كان "notes" — Backend يتوقع "description"
};
...
export default function DonationForm(): JSX.Element {
  ...
  const handleSubmit = useCallback(async (e?: React.FormEvent) => {
    ...
    try {
      ...
      fd.append('quantity', String(form.quantity));
      fd.append('condition', form.condition);
      // ✅ FIX Bug #3: كان fd.append('notes', ...) — الصح هو 'description'
    ...
    <main className="donation-page" dir="rtl" lang="ar">
      <div className="donation-card animate-up">
        ...
        <form className="donation-form" onSubmit={handleSubmit} noValidate>
          ...
          {/* ✅ FIX Bug #3: id وname وvalue وonChange كلها تستخدم "description" */}
          <div className="grid">
            ...
            <textarea
              ...
              rows={4}
            />
            <div className="notes-meta">{form.description.length}/500</div>
          </div>
