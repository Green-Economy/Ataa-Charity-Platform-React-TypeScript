  ...
  return (
    <>
      <header className="ap-topbar">
        ...
        {/* Desktop logout */}
        <button className="ap-topbar-logout" onClick={onLogout} title="تسجيل الخروج">
...
export default function AdminPanel() {
  ...
  const [cronLoading, setCronLoading] = useState<{ report: boolean; reminder: boolean }>({ report: false, reminder: false });
  const [cronLog,     setCronLog]     = useState<{ type: 'success' | 'error'; text: string; time: string }[]>([]);
  const [lastRun,     setLastRun]     = useState(0);
  ...
  // ── Cron ──────────────────────────────────────────────────────────────────
  const runAdminReport = async () => {
    setCronLoading(prev => ({ ...prev, report: true }));
    try {
      await apiFetch('/cron/adminReport');
      setLastRun(Date.now());
      setCronLog(prev => [{
      ...
      showMsg('success', 'تم تشغيل تقرير الأدمن بنجاح');
    } catch (err: unknown) {
      const errText = (err instanceof Error ? err.message : null) || 'فشل التشغيل';
      setCronLog(prev => [{
        type: 'error', text: errText,
        time: new Date().toLocaleTimeString('ar-EG'),
      }, ...prev]);
      showMsg('error', errText);
    } finally { setCronLoading(prev => ({ ...prev, report: false })); }
  };

  const runDonationReminder = async () => {
    setCronLoading(prev => ({ ...prev, reminder: true }));
    try {
      await apiFetch('/cron/donationReminder');
      setCronLog(prev => [{
        type: 'success', text: 'تم إرسال تذكيرات التبرعات',
      ...
      showMsg('success', 'تم إرسال تذكيرات التبرعات بنجاح');
    } catch (err: unknown) {
      const errText = (err instanceof Error ? err.message : null) || 'فشل التشغيل';
      setCronLog(prev => [{
        type: 'error', text: errText,
      ...
      showMsg('error', errText);
    } finally { setCronLoading(prev => ({ ...prev, reminder: false })); }
  };
              ...
              <div className="ap-tab-content ap-automation-wrap">
                ...
                <div className="cron-grid">
                  <CronCard
                    ...
                    code="GET /cron/donationReminder"
                    codeBg="#f0fdf4" codeBorder="#bbf7d0" codeColor="#166534"
                    loading={cronLoading.reminder} btnColor={TEAL2} onRun={runDonationReminder}
                  />
                  <CronCard
                    icon="ti-report-analytics" iconBg="#eff6ff" iconColor="#1e40af"
                    title="تقرير الأدمن"
                    ...
                    code="GET /cron/adminReport"
                    codeBg="#eff6ff" codeBorder="#bfdbfe" codeColor="#1e40af"
                    loading={cronLoading.report} btnColor="#1e40af" onRun={runAdminReport}
                  />
                </div>
